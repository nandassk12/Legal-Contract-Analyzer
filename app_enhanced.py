...
class LegalContractAnalyzer:
    ...
    def analyze_with_ai(self, preprocessed_data: Dict, analysis_depth: str = "Standard") -> ContractAnalysis:
        """Analyze contract using Claude API if available, otherwise fall back to rule-based analysis."""
        
        # Attempt to use Claude API if client is initialized
        if self.client:
            st.info("Using Claude API for analysis...")
            analysis_result = self._call_claude_api(preprocessed_data.get("cleaned_text", ""), analysis_depth)
            if analysis_result:
                return analysis_result
            else:
                st.warning("Claude API analysis failed. Falling back to rule-based system.")
...
def main():
    ...
    st.title("⚖️ Legal Contract Analysis Bot")
    ...
    with st.sidebar:
        ...
        ai_provider = st.selectbox("AI Provider", ["rule-based", "claude"], 
    ...
    if 'analyzer' not in st.session_state:
        ...
        # Update the analyzer with the new api_key and ai_provider from the sidebar
        st.session_state.analyzer = LegalContractAnalyzer(api_key, ai_provider)
            ...
            if uploaded_file and st.button("🔍 Analyze Contract", type="primary", use_container_width=True):
                ...
                try:
                    ...
                    audit_entry = AuditEntry(
                        ...
                        ai_provider=ai_provider if api_key and ai_provider == 'claude' else "rule-based",
                    ...
                    if analysis.compliance_issues:
                        st.subheader("⚖️ Legal Compliance Issues")
                    ...
                    with col_export1:
                        ...
                        try:
                            ...
                            st.download_button(
                                ...
                                file_name=f"contract_analysis_report_{analysis_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                    ...
                    with col_export2:
                        ...
                        st.download_button(
                            ...
                            file_name=f"contract_analysis_{analysis_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
...
*This is a basic template. Please consult with a legal professional for customization.*
    ...
    <div style='text-align: center'>
        <p><strong>⚖️ Legal Contract Analysis Bot</strong> - Empowering Indian SMEs with AI-Driven Contract Intelligence</p>
        <p>🔍 Analyze • 🛡️ Assess • 📋 Report • 💡 Recommend • 📊 Track</p>
        <p><em>⚠️ Disclaimer: This tool provides general guidance only. Always consult qualified legal professionals for important contracts.</em></p>
    </div>
